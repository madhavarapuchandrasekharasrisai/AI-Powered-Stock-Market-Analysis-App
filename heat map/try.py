...
def get_market_details_groq(index_name, df_summary):
    """Use Groq API to generate market details summary."""
    # Securely retrieve GROQ_API_KEY from Streamlit secrets or environment
    ...
    prompt = f"""
            Summarize today’s performance of the {index_name} index on NSE using this data:
            {df_summary}
...
def get_total_market_insights(df):
    """Compute aggregated total market insights using yfinance, handling missing 'yf_symbol'."""
    insights = {}
    # Total Market Cap (sum of ffmc)
    insights['Total Market Cap (Cr)'] = df['ffmc'].sum()

    # Number of Stocks
    insights['Number of Stocks'] = len(df)

    # Volatility (Std Dev of pChange)
    insights['Volatility (Std Dev of pChange)'] = round(df['pChange'].std(), 2)

    # Advances/Declines Ratio - Fixed with len() to avoid tuple issues
    advances = len(df[df['pChange'] > 0])
    declines = len(df[df['pChange'] < 0])
    insights['Advances/Declines Ratio'] = round(advances / declines, 2) if declines != 0 else 'N/A'

    # Use 'symbol' column to build yfinance tickers if 'yf_symbol' missing
    ...
    insights['Average P/B Ratio'] = round(sum(pb_ratios) / len(pb_ratios), 2) if pb_ratios else 'N/A'
    insights['Average Dividend Yield (%)'] = round(sum(div_yields) / len(div_yields), 2) if div_yields else 'N/A'
    insights['Average Beta'] = round(sum(betas) / len(betas), 2) if betas else 'N/A'
...
def build_pie_chart(df):
    ...
    fig.update_layout(
        width=220, height=220, showlegend=False,
        annotations=[dict(
            text=f'A: {advances}<br>D: {declines}<br>N: {no_change}',
            x=0.5, y=0.5, font_size=16, showarrow=False, font_color='#333333'
        )],
...
if mode == "Single Index":
    ...
    if not df.empty:
        ...
        with header1:
            # Treemap
            fig = build_treemap(df, slice_factor, color_scale, height=900)
        ...
        if index_filter == 'NIFTY TOTAL MARKET':
            st.subheader("Total Market Overview (Inspired by Screener.in)")
        ...
        st.error("⚠️ Failed to fetch data for the selected index. Please try another or check your connection.")
        ...
        for i, idx in enumerate(selected_indices):
            with cols[i]:
                ...
                if not df.empty:
                    ...
                    fig = build_treemap(df, slice_factor, color_scale, height=600)
                    st.plotly_chart(fig, use_container_width=True)
                    ...
                    st.error(f"⚠️ No data for {idx}. Try another index.")
    else:
        st.info("Please select at least one index to compare. ✨")

st.markdown("---")
st.caption("Made with ❤️ by YERRA SAI RAJESH | Data sourced from NSE India and Yahoo Finance")
